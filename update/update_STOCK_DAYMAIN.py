...
select_lastest_day_cmd='SELECT trade_date, price_close FROM s'+sys.argv[1]+' ORDER BY trade_date DESC LIMIT 1'

select_last59_day_cmd='SELECT trade_money,trade_stock FROM s'+sys.argv[1]+' ORDER BY trade_date DESC LIMIT 59 '

init_single_stock_table_cmd='python ..\init\init_STOCK_DAYMAIN.py %s %s'
...
try:
	cur.execute(select_lastest_day_cmd)
except MySQLdb.Error, e:
	if int(e.args[0]) == 1146:
		print "Call init_STOCK_DAYMAIN %s"%(sys.agrv[1])
		log("Call init_STOCK_DAYMAIN %s"%(sys.agrv[1]))
		os.system(cmd%(sys.agrv[1],'single_stock00'))
	else:
		print "mysql error"
		log("mysql error %s"%(sys.agrv[1]))
	exit()

#get lastest day data in table
lastest_day =''
last_day_close='0'
...
endYear = today.year
#combine to str
lastestDay="%d/%02d/%02d"%(lastest_day.year,lastest_day.month,lastest_day.day)
...
for year in range(startYear,endYear+1):
	...
	for month in range(startMonth,endMonth+1):
		...
		for txt in tr[2:len(tr)]:
			...
			if lastestDay>=trade_date:
				continue
			#get last 59 data
			cur.execute(select_last59_day_cmd)
			count=58
			for money,stock in cur:
				MA_money[count]=money
				MA_stock[count]=stock
				count-=1
